...
tar_load(c(
  zinb_nb_effects, 
zinb_summary, 
zinb_zero_effects, zinb_nb_effects_duration, zinb_zero_effects_duration, zinb_summary_duration,
...
Alarm management strategies that focus on integrating multiple physiological indicators into one index may not improve alarm performance. It is possible that the actions taken in response to alarms in the IPI-enabled group mediated the effect on oxygen saturation, which potentially signals a safety benefit for using this approach for capnography alarm management. Further research is needed to evaluate this potentially beneficial impact on patient safety.
